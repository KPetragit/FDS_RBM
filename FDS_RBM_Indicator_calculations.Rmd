...
Petra- can we shorten the variable names for drinking water? some stat packages might cut them 
...
```
Add the calculation of crowding index.Overcrowded living conditions can facilitate the rapid transmission of infectious diseases due to close contact among individuals. (Smith, 2020). As a result, overcrowding may cause health issues, thus the shelter is not considered as livable.

Petra  - maybe we can call HH14_cat something else like crowdingab3 or crowding_cat
...
Petra - when we calculate the indicator; can we name with your official names, once we have time, we can also change for RMS scripts.

Petra - I think also we need to add table as pop groups; the one Thomas provide gives values at the end but I think it's important to check all indicators one by one with disaggregation variables - let's discuss - the table is not enough especially that we have different pop groups.
...
Petra - maybe we can give the official name that you have when we calculate the final indicator? so final indicator is called RBM10203 and to align with RBM we need to use either 1 2 or 1 0 both for RMS and FDS
...
Petra - the access is suspiciously high; it's because H08 is missing?
```{r}
main <- main %>%
...
Petra - this indicator needsa table as the denom and num are a bit more tricky as in the standard RBM one - maybe we can add that table here?

## 3.2b Core Impact Indicator


Petra - readd here the definitions for secondary erolment as it's a dropdown menu; it's good to have in both indicators 
...
Petra - above should we put 5 ( never walk alone and 98 into missing instead of no?) assuming 98 is refused to answer

Petra - same as above to add systematic tables to check results after each calculation
...
Petra - I like as below showing the value and label as below before calculation?
...
```
Petra - is it normal that very few people has access to safe toilet?
