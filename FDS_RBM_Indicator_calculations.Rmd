...
table(main$H01_1)##In the past 30 days, did you need to see a health profesionnal for any reason?
...
hhroster <- hhroster %>% 
  mutate(outcome1_3 = case_when(
    ...
    ID_09 == 1 & ID_13 == "A" & Legal_birth_certificate == 2 & Legal_ID_card == 2 & Legal_passport == 2 ~ 2, #Assigns 2 if the person has no primary, secondary and no other legal documents), 
    TRUE ~ NA))
...
```
Ilgi: This needs to re-done. There is a mistake in how the legal variables were incorporated. Magrith will double check and let me know once she updates it. Additionally, we should discuss on which answers from ID_09 and ID_13 would be considered as a legal document. 
...
Ilgi: I am missing the ID_VAR variable in the dataset, which gives us the information is someone is a returnee. Magrith will incorporate it to the datatset. 
...
hhroster <- hhroster %>% 
  mutate(outcome14_1 = case_when(
    ...
    ID_VAR == 4 & ID_09 == 1 & ID_13 == "A" & Legal_birth_certificate == 2 & Legal_ID_card == 2 & Legal_passport == 2 ~ 2, #Assigns 2 if the person has no primary and no other documents), 
    TRUE ~ NA_real_))
  
```
```{r, eval=FALSE}
hhroster <- hhroster %>% 
  mutate(outcome14_1 = labelled(outcome1_3,
                               labels = c(
                                 'Yes' = 1,
                                 'No' = 2
                               ),
