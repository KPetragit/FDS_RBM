...
**Denominator**: Total number of individuals of tertiary school age 

**Other sources**
1. [RBM 11.1 Outcome Indicator Guidance](https://intranet.unhcr.org/content/dam/unhcr/intranet/protection-operations/prog-hub/program-cycle/indictors/core/outcome/Core%20outcome%2011.1%20Proportion%20of%20young%20people%20enrolled%20in%20tertiary%20and%20higher%20education.pdf)
...
```
# <span style="color:blue; font-size:28px;"><strong>11.2 Core Outcome Indicator - Partially</strong></span>
**11.2 Proportion of children and young people enrolled in the national education system**
##RBM21102

This indicator is defined as the proportion of forcibly displaced and stateless students enrolled in the national education system at primary and secondary level. Enrolled refers to the Gross Enrolment Ratio rather than the Net Enrolment Ratio because it aims to  show what proportion of the forcibly displaced and stateless population is included in the national  education system, even if not at the correct age. National education system is to be intended as public schools recognised by the national authorities, i.e., their certification is recognised to access the next education level or, when relevant, the labour market. 

**Unit of observation**: Individual 

**Numerator**:  Number of students enrolled in national primary and secondary school education

**Denominator**:  Total number of individuals of primary and secondary school age 

**Other sources**
1. [RBM 11.2 Outcome Indicator Guidance](https://intranet.unhcr.org/content/dam/unhcr/intranet/protection-operations/prog-hub/program-cycle/indictors/core/outcome/Core%20outcome%2011.2%20Proportion%20of%20children%20and%20young%20people%20enrolled%20in%20national%20education%20system.pdf)
```{r}
table(hhroster$HH_Educ02a) ## if the respondent is currently going to school
table(hhroster$HH_Educ03)  ## grade the respondent is currently attending 
table(hhroster$HH_Educ04a) ## What type of school you going to? Governement school, UN or NGO, community school, religious or faith-based private school, non-religious private school or any other type of school?
table(hhroster$HH_Educ04b) ## Is the school that you are going accepted by the national authorities?
```
```{r}
#Create a variable specifying when individual is enrolled in primary or secondary school 
hhroster <- hhroster %>%
  mutate(edu_pri_sec = case_when(
    (HH_Educ02a == 1 & HH_Educ03 %in% 1:12 & (HH_Educ04a == 1 | HH_Educ04a %in% c(2,3,4,5,6,96,98) & HH_Educ04b == 1)) ~ 1,# Ensure to check possible grade levels. In these example answers 1 - 12 were considered for primary and secondary grade level. 
    (HH_Educ02a == 1 & HH_Educ03 %in% 1:12 & (HH_Educ04a == 1 | HH_Educ04a %in% c(2,3,4,5,6,96,98) & HH_Educ04b == 2)) ~ 2,
    TRUE ~ NA_real_
  ))
```
```{r}
...
hhroster <- hhroster %>%
  mutate(
    age_pri_sec = case_when(
      HH_04 >= 6 & HH_04 <= 16 ~ 1,  #Remember to adjust age levels based on the tertiary education age per country
      TRUE ~ 2
    )
  )
```
Ilgi: The indicator says" Proportion of children and young people, but the nominator is number of students enrolled (regardless of age) - seems counter intuitive. Secondly, if someone doesn't know in what kind of school they are going, but knows it is accepted by the natioinal authorities, would this be enough for a yes? 
For "no", do we need to account for cases where they are not enrolled at all? 


# <span style="color:blue; font-size:28px;"><strong>12.1 Core Outcome Indicator - Partially</strong></span>
**11.2 Proportion of people using at least basic drinking water services **
##RBM21201
...
# <span style="color:blue; font-size:28px;"><strong>12.2 Core Outcome Indicator - Partially</strong></span>
...
# <span style="color:blue; font-size:28px;"><strong>13.1 Core Outcome Indicator - Partially</strong></span>
...
table(main$FS03) #Do you, personally, have a credit card in the host country?
...
main <- main %>%
  mutate(outcome13_1 = case_when(
    age_selected > 18 & (FI01 == 1 | FI02 == 1 | FI03 == 1 | FI10 == 1) ~ 1,
    age_selected > 18 & FI01 == 2 & FI02 == 2 & FI03 == 2 & FI10 %in% c(2,3) ~ 2,
...
```
Ilgi: For FI10 we also have a response option: I don't know what mobile money is. Can be assume that if someone doesn't know what it is they also don't use it? 
Moreover, in all other cases, where respondents are below 18, or respond with don't know or refuse to answer, this has been put to NA. 

# <span style="color:blue; font-size:28px;"><strong>13.2 Core Outcome Indicator - Partially</strong></span>
**13.2 Proportion of people who self-report positive changes in their income compared to the prevoius year**
##RBM21302
