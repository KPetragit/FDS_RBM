...
Ilgi: I changed the calculation here to also include the need to see a health professional. Lets discuss 
...
table(main$H01_1)##In the past 30 days, did you need to see a health professional for any reason?
...
main <- main %>%
  mutate(
    healthcare = case_when(
      ...
      H01_1 == 2 ~ NA_real_, #Assigns 2 if they did not need to see a health professional
...
```
```{r, eval=FALSE}
table(hhroster$outcome1_3)
```
Ilgi: We should discuss on which answers from ID_09 and ID_13 would be considered as a legal document. 
...
```
```{r, eval=FALSE}
# Add labels to the new variable 'outcome16_1'
main <- main %>%
