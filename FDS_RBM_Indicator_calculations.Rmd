...
Petra - I was checking MICS and they have 6 datasets so not sure how to go with datasets - to merge them or not as they different weights and maybe either way let's find another name then hhroster as it's more than hhroster that we have in the dataset 
Ilgi: Noted. Lets discuss on Monday. In any case, we should make a decision soon so that we can adjust the scripts based on the number of datasets and also incorporate this aspects into the data preparation. 
...
Petra-should we link here the intranet? I saw that below you added; so we can add the same way everywhere
Ilgi: Do you mean the RBM guidance? I have added it
...
Petra - for the health indicator below, I think we use ind level so it codes a lot as they don't have access - can we maybe if someone from the HH needed access take it as for all HHs - or another option is put them into missing ( those who did not need access)
...
Petra- can we shorten the variable names for drinking water? some stat packages might cut them - Ilgi: Done
...
Petra  - maybe we can call HH14_cat something else like crowdingab3 or crowding_cat - Ilgi: Done
...
Petra - when we calculate the indicator; can we name with your official names, once we have time, we can also change for RMS scripts. - Ilgi: Here you mean the official names as in codes, example: RBM10202?
...
Petra - maybe we can give the official name that you have when we calculate the final indicator? so final indicator is called RBM10203 and to align with RBM we need to use either 1 2 or 1 0 both for RMS and FDS - Ilgi: Ok, I can change the names once/if they are finalized and accepted generally? For 0/1 or 1/2 - I agree, but I think this should also be reflected in the answer options in the data collection form itself, otherwise it can be confusing in my opinion. 
...
Petra - the access is suspiciously high; it's because H08 is missing? Ilgi: Possibly yes, I would say there can be many cases when someone would need to see a health professional but doesn't try due to various reasons, and not in all cases this would mean they have access. There are 416 cases when someone needed to see a hp, but did not try. 
```{r}
main <- main %>%
...
Petra - readd here the definitions for secondary erolment as it's a dropdown menu; it's good to have in both indicators - Ilgi: I added the definitions (apologies an overshight on my side).
...
Petra - above should we put 5 ( never walk alone and 98 into missing instead of no?) assuming 98 is refused to answer - Ilgi: 98 is don't know. In the meeting we discussed that 5 and 98 should be coded as 2 (No). This was also done like this in SSD. Let's rediscuss. 
...
Petra - I like as below showing the value and label as below before calculation? - Ilgi: Not sure I understand what you mean, should something be adjusted? 
...
```
Petra - is it normal that very few people has access to safe toilet? Ilgi: Based on the field observation while in Pak, in rural areas many household shared their toilet with other households, and the toilet was just a hole in the ground. We can do a bit more digging into this to check. 
...
Petra- agreed that if they don't know what it is, we can put the options into NA, I know that we can include above 15 so I wonder if it makes sense for FDS
Ilgi: For when they don't know what mobile money is, I proposed to put it in 2 as if they don't use it. Let's re discuss on Monday. For the age component - since the working age is 15 or above it could  make sense to also look at the bank account/mobile money from 15 onwards. And also since we collect this information for 15+. 
...
Petra - the below calculation is 0 1 - would be good to align all script also with RMS scripts ( I know I said this before :()) 
Ilgi - It was a typo on my side in only one line. I have changed it to 2. The final calculation was already set to 2. 
...
Petra- I love the structure above! We should also add table here
Ilgi: Great! Noted, lets discuss more on tables on Monday
...
Petra- as you mentioned the other day, we can check how to best cite the guidance pieces but the way it is looks good to me
...
Petra - it does not find the 'Government_assistance' variable seems like here is an issue - 
Ilgi: it was due to a spelling mistake, I fixed it. 