...
RA_adult <- RA_adult %>%
  mutate(
    healthcare = case_when(
      ...
      H01_1 == 1 & (H11 %in% c(10, 11) | H15d == 2) ~ 2, #Assigns 2 if they saw a traditional health or spiritual healer, or if they needed more than 1 hour to reach the health professional. 
...
HoH <- HoH %>%
mutate(Improved_dw_Loc = ifelse(Improved_dw == 1 & BD01a %in% c(1, 2),1,2))%>%
mutate(Improved_dw_Loc = labelled(Improved_dw_Loc,
                               labels = c(
                                 "Yes" = 1,
                                 "No" = 2
...
HoH <-HoH %>%
  mutate(RBM10202=case_when(
    shelter==2 | electricity==2 | Improved_dw_access==2 | Landslides_flooding==2 ~ 2,
...
roster <- roster %>% 
  mutate(RBM20103 = case_when(
    ...
    ID_09 %in% c(1, 8, 9, 11, 12, 13, 14) & ID_13 %in% c("A","H","I","K","L","M","N","OT")  & Legal_birth_certificate == 2 & Legal_ID_card == 2 & Legal_passport == 2 ~ 2, #Assigns 2 if the person has no primary, secondary and no other legal documents), 
    TRUE ~ NA))
...
roster <- roster %>% 
  mutate(RBM21401 = case_when(
    ...
    ID_VAR == 4 & (ID_09 %in% c(1, 8, 9, 11, 12, 13, 14) & ID_13 %in% c("A","H","I","K","L","M","N","OT")  & Legal_birth_certificate == 2 & Legal_ID_card == 2 & Legal_passport == 2) ~ 2, #Assigns 2 if the person has no primary, secondary and no other legal documents), 
    TRUE ~ NA))
...
HoH <- HoH %>%
  mutate(ownership_plot = case_when(
    ...
    Land02 == 2 ~ 2, #Assigns 2 if the HH does not own the plot they currently live on
...
HoH <- HoH %>%
  mutate(use_own_any_plot = case_when(
    ...
    Land01 == 2 & Land03 == 2 & Land04 == 2 ~ 2, #Assigns 2 if they don't use, own or hold use rights for any plot of land
    TRUE ~ NA_real_ )) #Assigns NA if the answer is "don't know" or "refuse to answer".
...
HoH <- HoH %>%
  mutate(residental_plot = case_when(
    ...
    !grepl("A", Land09_1) ~ 2)) #Assigns 2 if the use of plot 1 is not residential
```
...
HoH <- HoH %>%
  mutate(use_own_current_plot = case_when(
    ...
    Land05_1 == 2 & Land07_1 == 2 ~ 2,  #Assigns 2 if the HH does not have ownership nor use rights of the plot they live on
    Land05_1 %in% c(98,99) & Land07_1 == 2 ~ 2, #Assigns 2 if the HH does not have use rights of the plot they live on & they don't know/refuse      to answer to ownership rights
    TRUE ~ NA_real_ )) #Assigns NA if the answer is "don't know" or "refuse to answer".
...
HoH <- HoH %>%
  mutate(legal_documents = case_when(
    ...
    Land10_1 == 2 ~ 2, #Assigns 2 if the HH does not have legal ownership or use right documents for the plot they live on
    TRUE ~ NA_real_ )) #Assigns NA if the answer is "don't know" or "refuse to answer".
```
...
HoH <- HoH %>%
  mutate(plot_security = case_when(
    ...
    Land15_1 %in% c(3,4) ~ 2)) #Assigns 2 when the likelihood is "Somewhat likely" and "Very likely"
...
HoH <- HoH %>%
  mutate(tenure_land = case_when(
    ...
    use_own_any_plot == 1 & (residental_plot == 2 | use_own_current_plot == 2 | legal_documents == 2 | plot_security == 2) ~ 2, 
    use_own_any_plot == 2 ~ 2, 
...
HoH <- HoH %>%
  mutate(own_use_house = case_when(
    ...
    House01 == 2 & House02 == 2 ~ 2, #Assigns 2 if the HH does not have ownership or use rights of the house they live in
    House01 %in% c(99,98) & House02 == 2 ~ 2, #Assigns 2 if the HH does not have use rights and does not know/refuses to answer for ownership         rights
    TRUE ~ NA_real_ ))
#Ilgi, what do we do with cases where they don't have ownership rights, and they don't know/refuse to answer on use rights?     
...
HoH <- HoH %>%
  mutate(documents_house = case_when(
    ...
    House04 == 2 ~ 2, #Assigns 2 if the HH does not have legal documents for the ownership or use of the house
...
HoH <- HoH %>%
  mutate(expelled_house = case_when(
    ...
    House10 %in% c(3,4) ~ 2)) #Assigns 2 when the likelihood is "Somewhat likely" and "Very likely"
...
HoH <- HoH %>%
  mutate(tenure_house = case_when(
    ...
    own_use_house == 2 | documents_house == 2 | expelled_house == 2 ~ 2, 
    TRUE ~ NA_real_ )) #Assigns NA in all other cases
```
```{r, eval=FALSE}
...
HoH <- HoH %>%
  mutate(RBM21601= case_when(
    ...
    ownership_plot != 1 & (tenure_land == 2 | tenure_house == 2)  ~ 2, #Assigns 2 if HH does not own the plot of land they live on, and they don'    have secure land or household tenure.
    TRUE ~ NA_real_ )) #Assigns NA in all other cases
...
HoH <- HoH %>%
  mutate(Government_assistance = case_when(
    ...
    ScProtec01 == 1 & apply(HoH[programme_columns], 1, function(row) all(row != 1, na.rm = TRUE)) ~ 2, # Assignes a 2 if assistance received is not from the government
